...
async def db_get_candidates(db: AsyncSession, skip: int = 0, limit: int = 10) -> list[CandidateModel]:
    results = await db.execute(select(CandidateModel).offset(skip).limit(limit))
    return results.scalars().all()

async def db_update_candidate(db: AsyncSession, candidate_id: int, updated_candidate: CandidateUpdateSchema) -> CandidateModel | None:
    # 非同期でクエリを実行
    result = await db.execute(select(CandidateModel).filter(CandidateModel.id == candidate_id))
    db_candidate = result.scalars().first()
    if db_candidate:
        db_candidate.name = updated_candidate.name
        db_candidate.party = updated_candidate.party
        db_candidate.win_count = updated_candidate.win_count
        await db.commit()  # 非同期コミット
        await db.refresh(db_candidate)  # 非同期リフレッシュ
    return db_candidate
